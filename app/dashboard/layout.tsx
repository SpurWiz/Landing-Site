...
const workspaceLinks = [
  ...
  { href: "/dashboard/projects", label: "Projects", icon: FolderOpen, badge: 3 },
  { href: "/dashboard/reports", label: "Audit Reports", icon: BarChart3 },
  ...
  return (
    <div className="flex h-screen bg-[#f0f2f7] overflow-hidden">
      ...
      <motion.aside
        ...
        className="relative z-20 flex flex-col bg-[#0d1117] border-r border-white/[0.06] shrink-0 overflow-hidden"
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-4 py-5 border-b border-white/[0.06]">
          ...
          <motion.div variants={labelVariants} transition={{ duration: 0.2 }} className="overflow-hidden">
            ...
            <p className="text-white/40 text-[10px] mt-1 tracking-widest uppercase">Audit Platform</p>
          </motion.div>
        </div>
              ...
              return (
                <Link key={link.href} href={link.href}>
                  <motion.div
                    ...
                    className={`flex items-center gap-3 px-2 py-2 rounded-lg mb-0.5 transition-colors relative ${
                      active ? "bg-[#103fd5]/20" : ""
                    }`}
                  >
                    {active && (
                      <motion.div
                        ...
                        className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-[#103fd5] rounded-full"
                      />
                    )}
                    <Icon
                      size={16}
                      className={active ? "text-[#5d7cff] shrink-0" : "text-white/40 shrink-0"}
                    />
                    <motion.span
                      variants={labelVariants}
                      transition={{ duration: 0.2 }}
                      className={`text-[13px] flex-1 truncate ${active ? "text-white font-medium" : "text-white/50"}`}
                    >
                      {link.label}
                    </motion.span>
            ...
            {lensLinks.map((link) => {
              ...
              return (
                <Link key={link.href} href={link.href}>
                  <motion.div
                    whileHover={{ backgroundColor: "rgba(255,255,255,0.06)" }}
                    className={`flex items-center gap-3 px-2 py-2 rounded-lg mb-0.5 ${
                  ...
                  >
                    ...
                    <motion.span
                      variants={labelVariants}
                      transition={{ duration: 0.2 }}
                      className={`text-[13px] truncate ${active ? "text-white font-medium" : "text-white/45"}`}
                    >
                      {link.label}
                    </motion.span>
                  </motion.div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Topbar */}
        <header className="h-14 bg-white border-b border-[#e5e7eb] flex items-center px-6 gap-4 shrink-0 z-10">
          <div className="flex-1 flex items-center gap-3 max-w-sm">
            <div className="flex items-center gap-2 bg-[#f0f2f7] rounded-lg px-3 py-2 flex-1">
